...
const guideLabels: Record<Locale, string> = {
  en: "Read the complete laser cutting pricing guide →",
  "zh-hant": "閱讀完整雷射切割報價指南 →",
  "zh-hans": "阅读完整激光切割报价指南 →",
  de: "Vollständigen Kalkulationsleitfaden lesen →",
  ja: "レーザー加工見積ガイドを読む →",
  es: "Leer la guía completa de corte láser →",
  fr: "Lire le guide complet de découpe laser →",
  "pt-br": "Ler o guia completo de corte a laser →",
  ko: "레이저 절단 견적 가이드 읽기 →",
  it: "Leggi la guida completa sui preventivi per taglio laser →",
  nl: "Lees de complete gids voor lasersnij-offertes →",
  pl: "Przeczytaj kompletny poradnik wyceny cięcia laserem →",
};
...
export function LaserCuttingView({ locale }: { locale: Locale }) {
  ...
  return (
    <article className="tool-page">
      ...
      <LaserCuttingCalculator locale={locale} dictionary={dictionary} />
      <p className="shell tool-privacy-note">{copy.privacyNote}</p>
      <section className="section seo-content">
        <div className="shell seo-grid">
          <div>
            ...
            <Link className="seo-guide-link" href={localizedPath(locale, "guides/how-to-price-laser-cutting-jobs")}>
              {guideLabels[locale]}
            </Link>
          </div>
          <div>
