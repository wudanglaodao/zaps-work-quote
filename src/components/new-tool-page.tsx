...
export function NewToolPage({ kind, locale, path, guidePath, Icon }: { kind: NewToolKind; locale: Locale; path: string; guidePath: string; Icon: LucideIcon }) {
  ...
  return <article className="tool-page"><JsonLd data={structuredData} /><header className="shell tool-intro"><span className="tool-icon"><Icon aria-hidden="true" /></span><div><h1>{copy.heading}</h1><p>{copy.intro}</p></div></header><NewToolCalculator kind={kind} locale={locale} dictionary={dictionary} /><div className="shell tool-ad"><AdsenseSlot /></div><section className="section seo-content"><div className="shell seo-grid"><div><p className="section-kicker">Method</p><h2>{copy.methodTitle}</h2><p>{copy.methodBody}</p>{locale === "en" ? <Link className="seo-guide-link" href={`/${guidePath}`}>Read the pricing guide →</Link> : null}</div><div><h2>{copy.faqTitle}</h2><div className="faq-list">{copy.faq.map((entry) => <details key={entry.question}><summary>{entry.question}</summary><p>{entry.answer}</p></details>)}</div></div></div></section></article>;
}
