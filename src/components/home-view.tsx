...
export function HomeView({ locale }: { locale: Locale }) {
  ...
  const workflowLabels = ({
    ...
    ko: ["입력", "비용 및 마진", "견적"],
    it: ["Dati", "Costi + margine", "Preventivo"],
    nl: ["Invoer", "Kosten + marge", "Offerte"],
    pl: ["Wprowadzanie", "Koszt + marża", "Oferta"],
  } as const)[locale];
  const seeHowItWorks = ({
    en: "See how it works",
    "zh-hant": "查看運作方式",
    ...
    <div className="home-page">
      ...
      <section className="shell home-hero">
        <div className="home-hero-inner">
          ...
          <div className="home-hero-actions">
            <Link className="button primary" href={localizedPath(locale, "calculators/3d-print-cost-calculator")}>
              {dictionary.home.primaryCta}
            </Link>
            <Link className="home-text-link" href="#how-it-works">
              <span>{seeHowItWorks}</span>
      ...
      <section className="home-workflow-section" id="how-it-works">
        <div className="shell home-section-intro">
          <div>
            <p className="section-kicker">{dictionary.home.workflowKicker}</p>
            <h2>{dictionary.home.workflowHeading}</h2>
          </div>
          <p>{dictionary.home.workflowCopy}</p>
        </div>
        <div className="shell home-workflow-grid">
          <article className="home-workflow-card blue">
            <div className="home-workflow-scene">
              <div className="home-mini-window home-input-window">
                {["100%", "76%", "88%"].map((width) => (
                  <div className="home-input-line" key={width}>
                    <span />
                    <i style={{ width }} />
                    <b />
                  </div>
            ...
            <div className="home-workflow-caption">
              <span className="home-workflow-icon">
                <Box aria-hidden="true" />
              </span>
              <h3>{workflowLabels[0]}</h3>
              <b>01</b>
            </div>
          </article>
          <article className="home-workflow-card green">
            <div className="home-workflow-scene">
              <div className="home-mini-window home-margin-window">
                <div className="home-margin-ring">
                  <strong>34%</strong>
                </div>
                <div className="home-result-bars">
                  {[74, 54, 88, 42].map((width) => (
                    <span key={width}>
                      <i style={{ width: `${width}%` }} />
                    </span>
                  ))}
            ...
            <div className="home-workflow-caption">
              ...
              <h3>{workflowLabels[1]}</h3>
              <b>02</b>
            </div>
          </article>
          <article className="home-workflow-card amber">
            <div className="home-workflow-scene">
              <div className="home-mini-window home-quote-window">
                <div>
      ...
      <section className="home-tools-section" id="tools">
        <div className="shell home-section-intro">
          <div>
            ...
            <h2>{dictionary.home.toolsHeading}</h2>
          </div>
        ...
        <div className="shell home-tool-layout">
          <Link className="home-featured-tool" href={localizedPath(locale, "calculators/3d-print-cost-calculator")}>
            <div className="home-tool-preview">
              <Image
                src="/assets/zaps-work_homepage_workflow_preview.png"
                alt={tools[0].names[locale]}
                width={1600}
                height={900}
              />
            </div>
            <div className="home-featured-copy">
              <div>
                ...
                <h3>{tools[0].names[locale]}</h3>
                <p>{tools[0].summaries[locale]}</p>
              </div>
              <ArrowRight aria-hidden="true" />
            </div>
          </Link>
          <div className="home-secondary-tools">
            <div className="home-planned-tools">
              {secondaryTools.map((tool) => {
                ...
                return (
                  <Link className="home-planned-card" href={localizedPath(locale, `calculators/${tool.slug}`)} key={tool.slug}>
                    <div>
                    ...
                    <small>{dictionary.common.live}</small>
                  </Link>
                );
              })}
            </div>
            <Link className="home-all-tools-link" href={localizedPath(locale, "calculators")}>
              <span>{dictionary.common.allTools}</span>
              <ArrowRight aria-hidden="true" />
            </Link>
          </div>
        </div>
        ...
        <div className="shell home-privacy-layout">
          <div className="home-privacy-visual" aria-hidden="true">
            <div className="privacy-browser-panel">
              <div className="privacy-panel-bar">
                <span>
                  <i />
                  <i />
                  <i />
                </span>
                <b>{dictionary.home.privacyBrowserLabel}</b>
                <LockKeyhole />
              </div>
              <div className="privacy-private-sheet">
                <div className="privacy-sheet-head">
                  <span>
                    <LockKeyhole />
                  </span>
                  <div>
                ...
                {[82, 64, 74, 56].map((width) => (
                  <div className="privacy-redacted-row" key={width}>
                    <span />
                    <i style={{ width: `${width}%` }} />
                  </div>
                ))}
                <div className="privacy-local-export">
                  <FileDown />
                  <span>PDF</span>
                  <span>CSV</span>
                </div>
            ...
            <div className="privacy-filter-gate">
              <ShieldCheck />
              <span>{dictionary.home.privacyBlockedLabel}</span>
            </div>
            <div className="privacy-metrics-panel">
              <div className="privacy-metrics-head">
                <Database />
                <span>{dictionary.home.privacyMetricsLabel}</span>
              </div>
              <div className="privacy-metric-chips">
                <span>PLA</span>
                <span>6.5h</span>
                <span>35%</span>
              </div>
              <div className="privacy-metric-chart">
                {[72, 46, 84, 58].map((width) => (
                  <span key={width}>
                    <i style={{ width: `${width}%` }} />
                  </span>
                ))}
          ...
          <div className="home-privacy-copy">
            ...
            <div className="home-privacy-points">
              <p>
                ...
                <span>{dictionary.home.privacyLocal}</span>
              </p>
              <p>
                <Database aria-hidden="true" />
                <span>{dictionary.home.privacyAnonymous}</span>
              </p>
            </div>
            <Link className="home-text-link home-privacy-link" href={localizedPath(locale, "privacy")}>
              <span>{dictionary.home.privacyCta}</span>
              <ArrowRight aria-hidden="true" />
            </Link>
          </div>
        </div>
