...
export function SiteHeader({ locale, dictionary }: { locale: Locale; dictionary: Dictionary }) {
  ...
  const alternatePath = pathname.replace(/^\/(en|zh-hant)(?=\/|$)/, `/${alternate}`);
  return (
    <header className="site-header">
      <nav className="shell nav" aria-label="Primary navigation">
        <Link className="brand" href={`/${locale}`}>zaps<span>.</span>work</Link>
        <div className="nav-links">
          <Link href={`/${locale}/tools`}>{dictionary.common.tools}</Link>
          <Link href={`/${locale}#how-it-works`}>{dictionary.common.howItWorks}</Link>
          <Link href={`/${locale}#guides`}>{dictionary.common.guides}</Link>
        </div>
        <div className="nav-actions">
          <Menu label={dictionary.common.language} value={localeLabel(locale)} icon={<Globe2 aria-hidden="true" />}>
            <Link className="preference-option selected" href={pathname}>{locale === "en" ? "English" : "繁體中文"}</Link>
          ...
          <Link className="button compact" href={`/${locale}/tools`}>{dictionary.common.allTools}</Link>
        </div>
      </nav>
    </header>
  );
