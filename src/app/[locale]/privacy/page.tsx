...
const baseCopy: Record<Exclude<Locale, "zh-hans">, PrivacyCopy> = {
  ...
  ko: { title: "개인정보 보호 | zaps.work", description: "zaps.work가 계산 및 분석 데이터를 처리하는 방식입니다.", heading: "개인정보 보호", intro: "계산기는 계정 없이 사용할 수 있습니다. 고객 정보를 모은 데이터베이스를 만들거나 유지하지 않습니다.", customerHeading: "고객 정보 및 견적 내용", customerBody: "회사명, 고객명, 이메일, 주소, 전화번호, 견적 번호, 항목 이름, 메모는 브라우저에 남습니다. Cloudflare D1 분석 데이터베이스로 전송되거나 저장되지 않습니다.", dataHeading: "기록하는 데이터", dataBody: "PDF 또는 CSV를 내보낼 때 도구, 언어, 통화, 재료 유형, 수량, 필라멘트, 제작 시간, 요율, 비용 내역, 견적 결과만 포함된 익명 허용 목록 스냅샷을 데이터베이스에 보냅니다. 자유 입력이나 고객 관련 데이터는 포함되지 않습니다.", analyticsHeading: "사이트 분석", analyticsBody: "페이지 방문, 유입 경로, 기기, 브라우저를 이해하기 위해 Vercel Analytics와 Google Analytics를 사용합니다. 견적 양식이나 고객 정보는 분석 이벤트 매개변수로 보내지 않습니다.", exportsHeading: "내보낸 문서", exportsBody: "PDF와 CSV 파일은 브라우저에서 생성되며 자동으로 업로드되거나 저장되지 않습니다." },
};
