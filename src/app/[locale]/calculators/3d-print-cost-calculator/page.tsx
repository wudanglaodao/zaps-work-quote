...
const guideLinkLabels = { en: "Read the complete 3D printing pricing guide →", "zh-hant": "閱讀完整 3D 列印定價指南 →", "zh-hans": "阅读完整 3D 打印定价指南 →", de: "Vollständigen Leitfaden zur 3D-Druck-Kalkulation lesen →", ja: "3Dプリント価格設定ガイドを読む →", es: "Leer la guía completa de precios de impresión 3D →", fr: "Lire le guide complet de tarification 3D →", "pt-br": "Ler o guia completo de precificação 3D →", ko: "3D 프린팅 가격 책정 가이드 읽기 →", it: "Leggi la guida completa sui prezzi di stampa 3D →", nl: "Lees de complete gids voor 3D-print prijzen →", pl: "Przeczytaj kompletny poradnik wyceny druku 3D →" } as const;
const searchContent = {
  en: [
...
export function ThreeDPrintView({ locale }: { locale: Locale }) {
  ...
  const structuredData = [
    {
      ...
      offers: { "@type": "Offer", price: "0", priceCurrency: ({ en: "USD", "zh-hant": "TWD", "zh-hans": "CNY", de: "EUR", ja: "JPY", es: "EUR", fr: "EUR", "pt-br": "BRL", ko: "KRW", it: "EUR", nl: "EUR", pl: "PLN" } as const)[locale] },
  ...
  return <article className="tool-page">
    ...
    <div className="shell holesnap-ad-shell"><HoleSnapAd locale={locale} /></div>
