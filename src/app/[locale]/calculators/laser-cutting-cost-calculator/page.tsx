...
const guideLabels: Record<Locale, string> = {
  en: "Read the complete laser cutting pricing guide →", "zh-hant": "閱讀完整雷射切割報價指南 →", "zh-hans": "阅读完整激光切割报价指南 →", de: "Vollständigen Kalkulationsleitfaden lesen →", ja: "レーザー加工見積ガイドを読む →", es: "Leer la guía completa de corte láser →", fr: "Lire le guide complet de découpe laser →", "pt-br": "Ler o guia completo de corte a laser →", ko: "레이저 절단 견적 가이드 읽기 →", it: "Leggi la guida completa sui preventivi per taglio laser →", nl: "Lees de complete gids voor lasersnij-offertes →", pl: "Przeczytaj kompletny poradnik wyceny cięcia laserem →",
};
...
export function LaserCuttingView({ locale }: { locale: Locale }) {
  ...
  const structuredData = [
    { "@context": "https://schema.org", "@type": "WebApplication", name: copy.heading, description: copy.description, url, applicationCategory: "BusinessApplication", operatingSystem: "Any", browserRequirements: "Requires JavaScript", offers: { "@type": "Offer", price: "0", priceCurrency: ({ en: "USD", "zh-hant": "TWD", "zh-hans": "CNY", de: "EUR", ja: "JPY", es: "EUR", fr: "EUR", "pt-br": "BRL", ko: "KRW", it: "EUR", nl: "EUR", pl: "PLN" } as const)[locale] }, inLanguage: htmlLanguage(locale) },
...
export default async function LaserCuttingPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  if (rawLocale === "en") permanentRedirect("/calculators/laser-cutting-cost-calculator");
